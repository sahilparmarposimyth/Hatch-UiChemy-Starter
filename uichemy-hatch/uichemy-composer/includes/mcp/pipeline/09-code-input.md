...
You were called because the user gave you **existing front-end code** — pasted HTML/CSS/JS in chat, a single `.html` file, or a local project folder — and wants it turned into a real WordPress page built out of Composer (UiChemy Composer) widgets. There is **no Figma URL** here. Unlike `start_site_build` (which invents a design from a one-line brief), your source of truth is **the code the user provided** — reproduce it faithfully, then adapt it to Composer conventions. Do NOT redesign it, restyle it, or "improve" it unless the user explicitly asked.

---

## Step 0 — Read this first

You are NOT pasting the user's code back into chat. Every section of their markup ends up as a real Elementor template on their WordPress site, via these tools:

- `check_config` — site readiness (call once, first)
- `get_globals_css` / `set_globals_css` — read/write the source's palette + type scale as the site's design-system CSS block (`#uichemy-globals`)
- `create_uichemy_composer_header_footer({ type:"header"|"footer", ... })` — Header / Footer via UiChemy's native Theme Builder (works on ANY Elementor, Pro/Nexter NOT required); site-wide by default, page-only only on explicit user request; see Build order
...
- `request_media_upload({ filename, mime })` — upload local media (images, SVG, or mp4/webm/ogg/mov video) so `<img src>` / `<video src>` becomes a real URL
...
If you feel the urge to print the converted HTML in chat instead of calling these tools — stop. The user wants a built page, not a code dump.
...
- **Header is section 1, Footer is the last section.** If the source has no header/footer, tell the user and proceed with what exists — do NOT invent a header/footer that wasn't in their code (that is `start_site_build`'s job, not this one).
...
If the source is genuinely one component (a single card, one hero), it's fine to build it as a single `create_uichemy_composer_page` widget — say so in the plan.
...
- `active_header[]` / `active_footer[]` — now includes UiChemy's own native templates too. If non-empty and you're about to build a header/footer, mention you'll be replacing the existing one (UiChemy keeps only one active template per type).
...
**This question is NOT optional and NOT skippable, and it has NO exception** — UiChemy's native engine always supports the "Entire site" option, regardless of what `header_footer_system` reports. Do not infer the answer from the user's original wording, do not assume "entire site" because that's the recommended default, and do not silently proceed with either path — always ask and WAIT for the reply before Step 3/4 build calls involving the header/footer. Body-only sections (no header/footer in the source) are unaffected — proceed normally if the source has neither.

If `active_header[]` / `active_footer[]` from Step 2 already has an entry, mention in the same message that choosing "Entire site" will replace it (UiChemy keeps only one active template per type).
...
2. Build the COMPLETE block (existing entries + the source's new ones) and call `set_globals_css`:
...
- **Images** — see Step 5. Never keep `data:` URIs, `blob:` URIs, local `./img/x.png` paths, or unreachable URLs in the final `<img src>`. Either re-home local images through `request_media_upload`, keep an already-public HTTPS URL the source used, or convert a purely decorative image to the CSS-only equivalent the source implied.
...
- **Scope every rule under `.uichemy-{slug}-{index}`.** Prefix the source's selectors so they only apply inside this widget. Global resets the source did on `body` / `html` / `*` become scoped resets (see the required block below). No `@import`, no `<link>` in widget CSS — Google Fonts go through `site_before_head` once on the first upload.
 - **If `uichemy-composer/code-file` appears in your ability list**, a vendor or framework stylesheet the source links (Bootstrap, a Tailwind build, an icon font such as Font Awesome) does not have to be scope-rewritten into every section, nor dropped. Upload it with `action: "request-upload"` (the bytes go disk to disk - do not read the file into context), then reference the returned path with a `<link>` in site-wide code (`uichemy-composer/platform`) or the page's own (`action: "set-page-code"`). It stays global and unscoped, which is what such a stylesheet expects - scoping it per section is what breaks it. An icon font also needs its `.woff2` / `.ttf` files uploaded through `uichemy-composer/media`, or every glyph renders as a box. The same applies to one large stylesheet several sections share. If the ability is not listed, reproduce only the rules the sections actually use and tell the user what you left out. Section-specific CSS still lives in the section when splitting it that way is clean. Two more cases also go to a code file even when nothing is shared or vendored: (1) the source CSS is **not cleanly divisible per section** - rules are cross-coupled across sections, or reproducing them faithfully as separate per-widget blocks would be fragile or lossy - upload the stylesheet whole rather than force a bad split; (2) the source **already hands you CSS as a separate file** (a project folder's `style.css`, a build's output stylesheet) rather than inline per section - keep it as a file and upload it as-is instead of manually re-distributing its rules into each widget.
...
- If the source depends on a **heavy** external JS library (Swiper, Lottie, full GSAP timelines, jQuery), tell the user it needs to be enqueued separately. Do NOT paste a CDN `<script>` into the widget. But a **small** effect (fade-in-on-scroll, parallax, count-up, marquee) is a few lines of vanilla — reimplement it, don't drop it (see "Page-global visuals" below for effects that span the whole page).
 - **If `uichemy-composer/code-file` appears in your ability list**, that heavy library is not a dead end. Upload the vendor `.css` / `.js` from the project folder with `action: "request-upload"` (the bytes go disk to disk - do not read the file into context), then reference the returned path with a `<link>` / `<script src>` in site-wide code (`uichemy-composer/platform`) or the page's own (`action: "set-page-code"`), and load it BEFORE the section JS that depends on it. The same applies to a large stylesheet the project ships and several sections share. If the ability is not listed, fall back to asking the user to enqueue it. Per-section JS still belongs in the section when splitting it that way is clean. As with CSS above, two more cases also go to a code file even when nothing is shared or vendored: the source JS is **not cleanly divisible per section** (shared state, cross-section references, one script driving several sections at once) - upload it whole rather than force a fragile split; or the source **already hands you JS as a separate file** rather than inline per section - keep it as a file and upload it as-is.
...
Step 2.5 is mandatory when the source has a header/footer, so by this point you already have `hf_placement` (`"site"` | `"page"`). Branch on THAT, never on `header_footer_system`, and never guess or skip ahead.

#### Case A — `hf_placement` is `"site"` (native Theme Builder path)

No `system` argument needed — `create_uichemy_composer_header_footer` is always UiChemy's native engine (the `system` argument on this tool is deprecated/ignored):

```
1. create_uichemy_composer_header_footer({ type:"header",
                                           title:"<Brand> Header", html, css, js,
                                           site_css:"<Google Fonts links from the source — only on this first call>",
                                           upload_images:true })
2. create_uichemy_composer_page({ title:"<Page title>", status:"draft", label:"<first body label>",
...
4. create_uichemy_composer_header_footer({ type:"footer",
                                           title:"<Brand> Footer", html, css, js,
                                           upload_images:true })
```

Header/Footer NEVER go through `create_uichemy_composer_page` here. Confirm each response has `active:true` and `system:"uichemy_native"`.
...
```
1. create_uichemy_composer_page({ title:"<Page title>", status:"draft", label:"Header",
                                  html, css, js,
...
Skip `create_uichemy_composer_header_footer` entirely in this case. (If the source had no header/footer at all, just build the body sections in order.)

Save from each response: `post_id` (after the first call — reuse it in every subsequent `add_uichemy_composer_section`), `active`, `preview_link`.

---

## Step 5 — Media: re-home the source's assets (images AND video/audio)

The upload tools sideload media by **fetching the URL over HTTP from the WP server.** They cannot read `data:` URIs, `blob:` URIs, local paths (`./assets/hero.png`, `./assets/demo.webm`), or any URL the server cannot reach. This applies to **every** media reference — `<img src>`, `srcset`, `<video src>`, `<source src>`, `<video poster>`, `<audio src>`, and `url(...)` backgrounds — not just `<img>`. So for each media file the source references:
...
2. **Local file** (in `project_path`, or a path the source uses) → upload it:
   1. Call `request_media_upload({ ttl_minutes })` **once for the whole build** → response has `slot_url`, `request_header`, `usage_example`, `batch_usage_example`, `expiry`.
   2. Run `batch_usage_example` via bash as ONE command: it loops over every file with `--data-binary @…`. Do NOT call the tool again per image.
...
**Asset import manifest (mandatory).** Every `create_uichemy_composer_page` / `add_uichemy_composer_section` response returns an `image_failures` array — each entry `{ from, error, code, source }` is a media reference (image / video / audio, in HTML `src`/`poster` or CSS `url()`) the server could **not** fetch. Collect them across ALL upload calls, dedupe by `from`, and if any remain, print a manifest so the user never has to discover a broken or placeholder asset by eye:

```
⚠️  Couldn't import N asset(s) — these still point at unreachable/local URLs:
    • assets/demo.webm      → local path (re-home via request_media_upload)
    • https://cdn.x/y.mp4   → fetch failed (403)
    Fix: upload each local file with request_media_upload (images + video/audio),
         or give me a public URL, and I'll re-run the affected section(s).
```

Do NOT silently leave a failed asset as a placeholder or mock without listing it here. `code = uich_local_asset` (or an invalid-URL code) → a local/relative path, fix is `request_media_upload`; a fetch/HTTP error → the source URL isn't publicly reachable.

---

## Anti-patterns — do not do these

- **Redesigning instead of converting.** Swapping the source's colors, fonts, spacing, or copy for your own. This flow is faithful reproduction — use `start_site_build` when the user wants you to invent a design.
- Pasting the source's raw code into chat instead of calling the upload tools.
...
- Putting header/footer markup inside `create_uichemy_composer_page` when `hf_placement` is `"site"`.
- Calling `create_uichemy_composer_header_footer` when `hf_placement` is `"page"` — use the in-page path instead.
...
- Inventing a header/footer the source didn't have (that's `start_site_build`, not this flow).
