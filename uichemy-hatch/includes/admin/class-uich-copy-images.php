...
class Uichemy_Gutenberg_Image_Import {
	...
	public static function cross_copy_paste_media_import() {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( __( 'You do not have permission to do this.', 'uichemy' ), 403 );
		}

		$media_import = isset( $_POST['content'] ) ? wp_unslash( $_POST['content'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- JSON block/media payload; decoded then processed recursively.
		...
		if ( is_array( $media_import ) || is_object( $media_import ) ) {
			...
			$updated_media = self::array_recursively_data(
				$media_import,
				function ( $block_data ) {
	...
	public static function check_block_image_upload( $block_data ) {
		...
		if ( isset( $block_data['name'] ) && isset( $block_data['attributes'] ) ) {
			...
			// Spectra Image Block
			if ( 'uagb/image' === $block_data['name'] || 'core/image' === $block_data['name'] ) {
			...
			// Core Image Block
			elseif ( 'core/image' === $block_data['name'] ) {
				if ( isset( $block_data['attributes']['url'] ) && isset( $block_data['attributes']['id'] ) && ! empty( $block_data['attributes']['url'] ) ) {
					$result                          = self::upload_media( $block_data['attributes']['url'] );
			...
			elseif ( 'uagb/container' === $block_data['name'] ) {
			...
			elseif ( 'core/group' === $block_data['name'] ) {
			...
			// Kedence Image Block
			elseif ( 'kadence/image' === $block_data['name'] ) {
			...
			elseif ( 'kadence/column' === $block_data['name'] ) {
			...
			elseif ( 'generateblocks/media' === $block_data['name'] ) {
			...
			elseif ( 'generateblocks/element' === $block_data['name'] ) {

				if ( isset( $block_data['attributes']['styles'] ) && isset( $block_data['attributes']['styles']['backgroundImage'] ) ) {
					$link   = str_replace( array( 'url(', ')' ), '', $block_data['attributes']['styles']['backgroundImage'] );
	...
	public static function upload_media( $url ) {
		...
		$existing_image = self::find_existing_image_attachment( array( 'url' => $url ) );

		if ( $existing_image ) {
			return array(
				'url'     => $existing_image['url'],
				'post_id' => (int) $existing_image['id'],
			);
		}
	...
	private static function find_existing_image_attachment( $attachment ) {
		...
		$post_id = $wpdb->get_var(  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct DB query intentional; caching not applicable.
			$wpdb->prepare(
