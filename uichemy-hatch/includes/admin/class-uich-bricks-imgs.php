...
class Uich_Bricks_Import_Images {
	...
	public static function import_media() {
		...
		if ( ! is_user_logged_in() || ! current_user_can( 'edit_pages' ) || ! current_user_can( 'upload_files' ) ) {
			wp_send_json_error( array( 'content' => __( 'Insufficient permissions.', 'uichemy' ) ), 401 );
		}

		$decode_data = isset( $_POST['inputData'] ) ? json_decode( wp_unslash( $_POST['inputData'] ), true ) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- JSON template payload; decoded then processed.

		$post_content = isset( $decode_data ) && ! empty( $decode_data ) && isset( $decode_data['content'] ) ? $decode_data['content'] : array();
	...
	public static function import_bricks_media( $content = array() ) {
