...
`ALLOWED_IMG_ORIGINS` and `HATCH_IMG_ALLOWED_HOSTS` sound alike and are not
the same thing. The first gates **this service's** own `/img` resizer. The
second is baked into **the site you deploy**, whose `/img` route allowlists an
upstream host before forwarding the resize here — and because `PUBLIC_` is
